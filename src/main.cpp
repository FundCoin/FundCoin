...
int64 nChainStartTime = 1407290400;
...
int64 GetProofOfWorkReward(int nHeight, int64 nFees, uint256 prevHash)
{
    ...
    if(nHeight == 1){
	       nSubsidy = 1300000 * COIN; // first 1 blocks for 1.5%
	       return nSubsidy + nFees;
	  }
	  else if(nHeight >= 2 && nHeight <= 18720){
	  	   nSubsidy = 6875 * COIN; // 88coins for per block
	  	   return nSubsidy + nFees;
	  }
...
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nBits, unsigned int nTime, int nHeight)
{
    int64 nRewardCoinYear;
		...
		if(nHeight < YEARLY_BLOCKCOUNT)
			nRewardCoinYear = 0 * MAX_CLOAK_PROOF_OF_STAKE;
		else if(nHeight < (0 * YEARLY_BLOCKCOUNT))
			nRewardCoinYear = 0 * MAX_CLOAK_PROOF_OF_STAKE;
		else if(nHeight < (0 * YEARLY_BLOCKCOUNT))
			nRewardCoinYear = 0 * MAX_CLOAK_PROOF_OF_STAKE;
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in FundCoin
    bool fStrictPayToScriptHash = true; // Always active in FundCoin
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "FundCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "FundCoin is good coin wawawa......";
        CTransaction txNew;
        ...
        assert(block.hashMerkleRoot == uint256("0x7f3635f75bdd1c030674a0db5f896d4d85d2b8a77bb891713b811306c27fa905"));
